...
export const NotificationIcon = ({ count,isDashboard=false }: { count: number ;isDashboard:boolean }) => {
  return (
    <div className={`relative ${isDashboard ? "text-white" : "text-black dark:text-white"} `}>
      {count ? (
        <div className="absolute -top-2 -right-2 text-xs rounded-full w-4 h-4 flex justify-center items-center bg-red-500">
          {count}
        </div>
      ) : null}
