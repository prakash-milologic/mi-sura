...
export async function GET(request: Request) {
  try {
    ...
    data = await tsdb.execute(
      sql`select time_bucket('1 day', z_timestamp) as time,
      trunc(max(dpy), 3) as "dailyYield", 
      trunc(max(tpy), 3) as "totalYield"
      from invt_75fe4cee i
      where z_timestamp > now() - interval '1 month' 
      and date_trunc('month', z_timestamp at time zone ${currentTZ}) 
      = date_trunc('month', ${timeQuery}::timestamp)
      group by time
      order by time;`
    );
