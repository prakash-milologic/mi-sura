  ...
  try {
    ...
    data = await Promise.all(
      plants.map(async (plant) => {
        ...
        if (periodQuery === "day") {
          return (
            (await get_daily_report(devices, timeQuery)).at(0) ||
            plantWithNoData
          );
          ...
          return (
            (await get_yearly_report(devices, timeQuery)).at(0) ||
            plantWithNoData
          );
        } else if (periodQuery === "total") {
          return (await get_total_report(devices)).at(0) || plantWithNoData;
        }
      })
    );

    const initialSum = { currentYield: 0, totalYield: 0 };

    data = data?.reduce((acc: any, currObj: any) => {
      return {
        currentYield: Number(acc?.currentYield) + Number(currObj?.currentYield),
        totalYield: Number(acc?.totalYield) + Number(currObj?.totalYield),
      };
    }, initialSum);

    if (!data?.currentYield && !data?.totalYield) {
      data = { currentYield: null, totalYield: null };
    }
    ...
    //     sql`
    //           WITH invt_query AS (
    //             SELECT time_bucket('5 minutes', z_timestamp) AS tbucket, MAX(dpy) AS dpy, MAX(tpg * 0.1) AS tpy
    //             FROM invt_75fe4cee
    //             WHERE date_trunc('day',z_timestamp at time zone ${currentTZ}) = date_trunc('day',${timeQuery} at time zone ${currentTZ})
    //             GROUP BY tbucket
    //           ),
    //           sungrow_query AS (
    //               SELECT time_bucket('5 minutes', z_timestamp) AS tbucket, MAX(dpy * 0.1) AS dpy, MAX(tpy) AS tpy
    //               FROM sungrow
    //               WHERE date_trunc('day',z_timestamp at time zone ${currentTZ}) = date_trunc('day',${timeQuery} at time zone ${currentTZ})
    //               GROUP BY tbucket
    //           ),
    ...
    //         ),
    //         combined_result as (
    //             SELECT time_bucket('1 day', tbucket) as bucket, SUM(dpy) AS dpy, SUM(tpy) AS tpy
    //             FROM (
    //                 SELECT * FROM invt_query
    //                 UNION ALL
    //                 SELECT * FROM sungrow_query
